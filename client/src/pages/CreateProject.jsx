...
//   const sendInvite = async (developerId) => {
//   if (!projectId) {
...
function CreateProject() {
  ...
  // 🔥 NEW STATE — Track invited developers
  ...
  const sendInvite = async (developerId) => {
    ...
    try {
      ...
      // Add developer to invited state
      setInvitedDevelopers((prev) => [...prev, developerId]);
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {recommended.map((dev, index) => (
                ...
                >
                  ...
                  {/* 🔥 SMART INVITE BUTTON */}
                  <motion.button
                    whileHover={{ scale: invitedDevelopers.includes(dev._id) ? 1 : 1.03 }}
                    whileTap={{ scale: invitedDevelopers.includes(dev._id) ? 1 : 0.97 }}
                    onClick={() => sendInvite(dev._id)}
                    disabled={invitedDevelopers.includes(dev._id)}
                    className={`mt-4 w-full py-2 rounded-xl text-sm font-semibold transition
                      ${
                        invitedDevelopers.includes(dev._id)
